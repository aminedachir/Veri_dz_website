...
def _fetch_url_content(url: str) -> str:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            # Return first 3000 chars — enough for a headline + intro
            return text[:3000]
    except Exception:
        return ''
...
def analyze_with_claude(text: str, url: str = '') -> dict:
    ...
    if url:
        ...
        if fetched:
            ...
            user_content += f"(تعذّر جلب محتوى الصفحة — سيتم التحقق من الرابط فقط)\n\n"
    ...
    try:
        ...
        return {'error': f"خطأ في تحليل الاستجابة: {str(e)}"}
...
def _demo_response(text: str, url: str = '') -> dict:
    ...
    if any(w in combined for w in ['fake', 'faux', 'mensonge', 'rumeur', 'كذب', 'مزيف', 'شائعة']):
        ...
        confidence = 0.50
        explanation = "تعذّر جلب محتوى الرابط في وضع التجربة. قم بتهيئة ANTHROPIC_API_KEY للحصول على تحليل حقيقي مع قراءة محتوى الصفحة."
    else:
        verdict = 'unverifiable'
        confidence = 0.55
    ...
    return {
        ...
        "explanation": f"[وضع تجريبي — قم بتهيئة ANTHROPIC_API_KEY للتحليل الحقيقي]\n{explanation}",
