...
def factcheck_view(request):
    ...
    if request.method == 'POST':
        ...
        if not input_text and not input_url:
            ...
            # ✅ تحقق من وجود مفتاح Groq
            api_key = getattr(settings, 'GROQ_API_KEY', '')
            if not api_key:
                error = "⚠️ مفتاح Groq غير موجود. يرجى إضافته في متغيرات البيئة."
            else:
                # ✅ استخدم Groq بدلاً من OpenRouter
                analysis = analyze_with_groq(input_text, url=input_url)
